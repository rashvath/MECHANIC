...
export function DetailedBillModal({ open, booking, saving, onClose, onSave }: DetailedBillModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/65 backdrop-blur-[2px] p-3 sm:p-6">
      <Card className="mx-auto my-2 w-full max-w-5xl border-[#5b4424] bg-[#121821] p-4 shadow-[0_24px_60px_rgba(0,0,0,0.55)] sm:my-6 sm:p-6">
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <div className="flex gap-2">
            <Button variant="secondary" onClick={() => setShowPreview((v) => !v)}>
              {showPreview ? "Hide Bill" : "View Bill"}
            </Button>
          ...
          <div className="space-y-3">
            {items.map((item) => (
              <div key={item.id} className="grid gap-2 rounded-lg border border-[var(--border)] p-3 sm:grid-cols-[1.3fr_1.6fr_0.8fr_auto]">
                ...
                />
                <Button type="button" variant="ghost" onClick={() => removeItem(item.id)} className="self-center">
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-3">
          <div>
            ...
            <select
              ...
              className="h-11 w-full rounded-xl border border-[var(--border)] bg-[var(--muted)] px-3 text-sm"
            >
